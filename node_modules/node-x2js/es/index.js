/*
   based on https://github.com/abdmob/x2js/blob/master/xml2json.js, VERSION = "1.2.0"
 */
  ...
  function parseDOMChildren(node, path) {
    if (node.nodeType == DOMNodeTypes.DOCUMENT_NODE) {
      ...
      for (var _cidx = 0; _cidx < _nodeChildren.length; _cidx++) {
        var _child = _nodeChildren.item(_cidx); // nodeChildren[cidx];
        _childName = getNodeLocalName(_child);

        if (_child.nodeType != DOMNodeTypes.COMMENT_NODE) {
          var childPath = path + "." + _childName;
          if (checkXmlElementsFilter(_result, _child.nodeType, _childName, childPath)) {
            _result.__cnt++;
            if (_result[_childName] == null) {
              _result[_childName] = parseDOMChildren(_child, childPath);
              toArrayAccessForm(_result, _childName, childPath);
            } else {
              if (_result[_childName] != null) {
                if (!(_result[_childName] instanceof Array)) {
                  _result[_childName] = [_result[_childName]];
                  toArrayAccessForm(_result, _childName, childPath);
                }
              }
              _result[_childName][_result[_childName].length] = parseDOMChildren(_child, childPath);
            }
          }
        }
      }
      ...
      for (var aidx = 0; aidx < node.attributes.length; aidx++) {
        ...
        _result.__cnt++;
        _result[config.attributePrefix + attr.name] = attr.value;
      }
      ...
      if (_result["#text"] != null) {
        ...
        if (config.stripWhitespaces) _result.__text = _result.__text.trim();
        delete _result["#text"];
        if (config.arrayAccessForm == "property") delete _result["#text_asArray"];
        _result.__text = checkFromXmlDateTimePaths(_result.__text, _childName, path + "." + _childName);
      }
      if (_result["#cdata-section"] != null) {
        _result.__cdata = _result["#cdata-section"];
        delete _result["#cdata-section"];
        if (config.arrayAccessForm == "property") delete _result["#cdata-section_asArray"];
      }

      if (_result.__cnt == 0 && config.emptyNodeForm == "text") {
        _result = '';
      } else if (_result.__cnt == 1 && _result.__text != null) {
        _result = _result.__text;
      } else if (_result.__cnt == 1 && _result.__cdata != null && !config.keepCData) {
        _result = _result.__cdata;
      } else if (_result.__cnt > 1 && _result.__text != null && config.skipEmptyTextNodesForObj) {
        if (config.stripWhitespaces && _result.__text == "" || _result.__text.trim() == "") {
          delete _result.__text;
        }
      }
      delete _result.__cnt;

      if (config.enableToStringFunc && (_result.__text != null || _result.__cdata != null)) {
